            ...
            Object.keys(picks).map(pick=>{
                ...
                return(
                    <div className="flex flex-row items-center gap-3 text-sm font-semibold text-slate-300">
                        {name!=null && <div className='rounded-full max-w-[80px]'><ChampProfile onClick={()=>{assignChamp(pick)}} name={name}></ChampProfile></div>}
                        {name==null && <div className='rounded-full max-w-[80px]'><div onClick={()=>{assignChamp(pick)}} className="flex w-20 h-20 rounded-full bg-slate-700 hover:shadow-md hover:shadow-slate-400"></div></div>}
